...
public class ReportExportController {
    ...
    public void exportRequestsCsv(
        ...
        writer.println(csv("Report generated by " + currentUserLabel() + " on "
        ...
        for (Request r : filterByDate(from, to, principal)) {
    ...
    public void exportTechnicianPerformanceCsv(
        ...
        scopedTechnicians(principal).forEach(t -> writer.println(String.join(",",
    ...
    public void exportCategoriesCsv(
        ...
        List<Request> requests = filterByDate(from, to, principal);
        requests.stream()
    ...
    public void exportMonthlyVolumeCsv(
        ...
        filterByDate(from, to, principal).stream()
    ...
    public void exportTechnicianPerformancePdf(
        ...
        var technicians = scopedTechnicians(principal);
    ...
    public void exportMonthlyVolumePdf(
        ...
        List<Request> all = filterByDate(from, to, principal);
    ...
    public void exportAnalyticsPdf(
        ...
        // ── Gather data (district-scoped automatically for STAFF) ───────────
        Map<String, Object>        aiSummary    = reportService.aiSummary();
        List<Map<String, Object>>  aiAccuracy   = reportService.aiAccuracy();
        List<Map<String, Object>>  byPriority   = reportService.byPriority(principal);
        List<Map<String, Object>>  byCategory   = reportService.byCategory(principal);
        List<Map<String, Object>>  byStatus     = reportService.byStatus(principal);
        List<Map<String, Object>>  byProvince   = reportService.byProvince(principal);
        List<Map<String, Object>>  techPerf     = reportService.technicianPerformance(principal);
        Map<String, Object>        sla          = reportService.slaMetrics(principal);
        List<Map<String, Object>>  monthly      = reportService.monthlyVolume(months, principal);
        ...
        Paragraph rByLine = new Paragraph("Generated by: " + currentUserLabel(), df); rByLine.setAlignment(Element.ALIGN_RIGHT);
        right.addElement(rt); right.addElement(rd); right.addElement(rByLine);
        ...
        List<Technician> visibleTechs = scopedTechnicians(principal);
        long totalTechs    = visibleTechs.size();
        long availableTechs= visibleTechs.stream()
                .filter(t -> Boolean.TRUE.equals(t.getIsAvailable())).count();
        long totalRes      = visibleTechs.stream()
    ...
    public void exportFullReportPdf(
        ...
        // District-scoped automatically for STAFF, global for ADMIN
        List<Map<String, Object>> volumeData   = reportService.monthlyVolume(months, principal);
        List<Map<String, Object>> priorityData = reportService.byPriority(principal);
        List<Map<String, Object>> categoryData = reportService.byCategory(principal);
        List<Map<String, Object>> sectorData   = reportService.bySector(principal);
        List<Map<String, Object>> techData     = reportService.technicianPerformance(principal);
        ...
        Paragraph rb = new Paragraph("Generated by: " + currentUserLabel(), df); rb.setAlignment(Element.ALIGN_RIGHT);
        right.addElement(rt); right.addElement(rd); right.addElement(rb);
        banner.addCell(right);
        doc.add(banner);
        doc.add(Chunk.NEWLINE);
        ...
        // ── SLA Health ──────────────────────────────────────────────────────
        doc.add(subTitle("SLA Health"));
        long slaTotalResolved = ((Number) sla.getOrDefault("totalResolved", 0L)).longValue();
        addSummaryRow(doc, new String[][]{
                {"Breached",   String.valueOf(sla.getOrDefault("breached", 0))},
                {"At Risk",    String.valueOf(sla.getOrDefault("atRisk", 0))},
                {"Resolved Within SLA", String.valueOf(sla.getOrDefault("resolvedWithinSla", 0))},
                {"Within SLA Rate", slaTotalResolved > 0
                        ? String.format("%.0f%%", ((Number) sla.getOrDefault("withinSlaRate", 0.0)).doubleValue())
                        : "—"}
        });
        ...
        doc.add(sectionTitle("5 · Sector Breakdown"));
        if (!sectorData.isEmpty()) {
            String[] gh = {"Sector", "Total Requests"}; float[] gw = {260, 120};
            PdfPTable gt = createTable(gh, gw); int[] gi = {0};
            sectorData.forEach(r -> addRow(gt, gi, new String[]{
                    String.valueOf(r.get("sector")), String.valueOf(r.get("count"))}));
            doc.add(gt);
        } else {
            doc.add(new Paragraph("No sector data available.", FONT_TD));
        }
        doc.add(Chunk.NEWLINE);

        // ── Section 6: Technician Performance ──────────────────────────────
        doc.newPage();
        doc.add(sectionTitle("6 · Technician Performance"));
        if (!techData.isEmpty()) {
            String[] th = {"Name", "Assigned", "Resolved", "Resolution Rate", "Avg Hours", "Rating", "Workload"};
            float[]  tw = {160, 60, 60, 80, 65, 55, 65};
            ...
            techData.stream()
                    ...
                    .forEach(r -> {
                        ...
                        long resolved = ((Number) r.get("totalResolved")).longValue();
                        String rate   = assigned > 0 ? String.format("%.0f%%", resolved * 100.0 / assigned) : "—";
                        Object avgH   = r.get("avgResolutionHours");
                        Object rating = r.get("rating");
                        Object cur    = r.get("currentWorkload");
                        Object max    = r.get("maxWorkload");
                        addRow(tt, ti, new String[]{r.get("firstName") + " " + r.get("lastName"),
                                String.valueOf(assigned), String.valueOf(resolved), rate,
                                avgH   != null ? String.format("%.1fh", ((Number) avgH).doubleValue()) : "—",
                                rating != null ? String.valueOf(rating) + "/5" : "—",
        ...
        // ── Section 7: AI Model Accuracy ───────────────────────────────────
        doc.add(sectionTitle("7 · AI Model Accuracy"));
        if (!aiData.isEmpty()) {
    ...
    /**
     * "Full Name (email)" of the authenticated user generating the report.
     * For STAFF the label also states the district scope, so exported documents
     * are self-describing about which subset of data they contain.
     */
    private String currentUserLabel() {
        var auth = SecurityContextHolder.getContext().getAuthentication();
        if (auth != null && auth.getPrincipal() instanceof UserPrincipal p) {
            String label = p.getFullName() + " (" + p.getEmail() + ")";
            String district = staffDistrict(p);
            if (district != null) {
                label += " · Scope: " + district + " District";
            }
            return label;
        }
        return "system";
    ...
    /**
     * District an export should be scoped to: the staff member's own district,
     * or {@code null} (global) for ADMIN and for staff without a configured
     * district — mirroring the request-list and report scoping rules.
     */
    private String staffDistrict(UserPrincipal principal) {
        if (principal == null || principal.getRole() != com.reg.arms.entity.enums.UserRole.STAFF) return null;
        String district = principal.getDistrict();
        return (district != null && !district.isBlank()) ? district : null;
    }

    /**
     * Technicians visible to the caller. STAFF see technicians working in their
     * district — including technicians with no district set, since those can be
     * assigned to any district's requests. ADMIN sees all.
     */
    private List<Technician> scopedTechnicians(UserPrincipal principal) {
        List<Technician> all = technicianRepository.findAll();
        String district = staffDistrict(principal);
        if (district == null) return all;
        return all.stream()
                .filter(t -> {
                    String techDistrict = t.getUser() != null ? t.getUser().getDistrict() : null;
                    return techDistrict == null || techDistrict.isBlank()
                            || techDistrict.equalsIgnoreCase(district);
                })
                .toList();
    }

    /**
     * A technician's coverage area for reports. Prefers the explicit
     * province_coverage list, but falls back to the technician's own profile
     * location — technicians created through the app inherit province/district
     * on their user profile and never have the jsonb coverage list populated.
     */
    private String coverage(Technician t) {
        if (t.getProvinceCoverage() != null && !t.getProvinceCoverage().isEmpty()) {
            return String.join(", ", t.getProvinceCoverage());
        }
        var u = t.getUser();
        if (u != null && u.getProvince() != null && !u.getProvince().isBlank()) {
            return (u.getDistrict() != null && !u.getDistrict().isBlank())
                    ? u.getProvince() + " — " + u.getDistrict()
                    : u.getProvince();
        }
        return "";
    }
    ...
    private void addHeader(Document doc, String title, LocalDate from, LocalDate to) throws DocumentException {
        ...
        Paragraph rBy    = new Paragraph("Generated by: " + currentUserLabel(), dateFont);
        rBy.setAlignment(Element.ALIGN_RIGHT);
        right.addElement(rTitle);
        right.addElement(rDate);
        right.addElement(rGen);
        right.addElement(rBy);
        banner.addCell(right);
    ...
    private List<Request> filterByDate(LocalDate from, LocalDate to, UserPrincipal principal) {
